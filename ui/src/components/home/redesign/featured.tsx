...
// Flag emoji per supported campaign country.
    ...
    <section className="bg-bg-off-white py-16 md:py-24">
      <div className="max-w-[1600px] mx-auto px-5 sm:px-8 lg:px-[clamp(40px,5vw,120px)]">
        ...
        <div className="space-y-6 md:space-y-8">
          {allCampaigns.map((c) => (
            <FeaturedCampaignCard key={c.id} campaign={c} />
          ))}
        </div>
      </div>
    </section>
...
function FeaturedCampaignCard({ campaign: c }: { campaign: Campaign }) {
  const { t } = useLanguage();
    ...
    <>
      ...
      >
        ...
        >
          ...
          {c.country && (
            <span className="absolute top-4 right-4 inline-flex items-center gap-1.5 bg-white/95 backdrop-blur px-3 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-wider text-primary-navy">
              <span aria-hidden className="text-[14px] leading-none">{COUNTRY_FLAGS[c.country] ?? "🏳️"}</span>
              {c.country}
            </span>
          )}
        </Link>
